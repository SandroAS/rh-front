...
interface EvaluationApplicationStoreState {
  ...
  pending_by_evaluator: PendingByEvaluatorResponse[] | null;
  loading: boolean;
  error: string | null;
  total: number;
  page: number;
...
export const useEvaluationApplicationStore = defineStore('evaluationApplication', {
  ...
  actions: {
    ...
    async getPendingByEvaluator() {
      ...
      try {
        const res = await getPendingByEvaluator();
        this.pending_by_evaluator = res;
      } catch (err: any) {
        this.error = err.response?.data?.message || 'Erro ao tentar buscar avaliações pendentes por avaliador.';
        throw err;
      } finally {
        this.loading = false;
      }
